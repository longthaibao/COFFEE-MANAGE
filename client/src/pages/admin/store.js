...
export default function StoreRevenue(){
    ...
    const nameStyle = {
        display: 'flex', fontWeight: 'bold', fontSize: '35px', marginLeft: "30px", alignItems: "start" ,
    };
    ...
    <>  
        <div>
          <div style={{ fontFamily: 'Montserrat', display: 'flex'}}>
              <div style={{width: "33.3333%", height: "160px", border: '1px solid black', marginTop: "20px", marginLeft: "50px"}}>
                  <div style={nameStyle} >
                      <FaCoffee style={{marginRight: "10px", fontSize: "65px",  marginTop: "20px"}}/>
                      <p style={{display: 'inline-block', marginTop: "20px", borderBottom: '4px double black'}}>PLACY COFFEE</p> 
                  </div>
                  <p style={{fontWeight: 'normal', fontSize: '20px', marginLeft: "110px"}}>{storeName()}</p>
              </div>
              <div style={{marginLeft: "300px", marginTop: "20px", fontWeight: 'bold'}}>
                  <div className = "flex">
                      ...
                      <div>
                        <input
                          ...
                          style={{ border: '1px solid', borderRadius: '4px', background: '#e3cd8e', color: "#0c0ee3"}}/>
                      </div>
                  </div>
                  ...
                  <div className = "flex" style={{marginTop: "10px"}}>
                    ...
                    {producFromTable
                      ...
                      .map(filteredItem => (
                        <div style = {{color: "#0c0ee3"}} key={filteredItem.product_ID}>
                          <p>{filteredItem.product_name} - {filteredItem.product_size} -{filteredItem.total_quantity_sold} </p>
                        </div>
                      ))}
                  </div>
              </div>
          ...
          {/* table */}
          <div>
            <ProRevTable startDate={selectedStartDate} endDate= {selectedEndDate} onDataFromChild={handleData}/>
          </div>
        </div>
    </>
  )
